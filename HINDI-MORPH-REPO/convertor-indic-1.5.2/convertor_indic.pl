			...
			if ($tkn !~ /^\^?\@.*/)
			{
			    $lex_out = &wx2utf($tkn, $lang);
        	        ...
        	        if($fs ne "")
                	{
        	                ...
        	                foreach $af (@fss)
                	        {
					#To handle issue if feature structure does't have af compiste key
					if ($af !~ /^\^<fs  *af=.*$/)
                                        {
                                                $af =~s /<fs /<fs af=',,,,,,,' /g;
                                        }
				...
				# delete the empty added af key from FS
				$newfs =~s /af=',,,,,,,'//g; 
				...
				#if($lex ne "((" and $lex ne "))") # comment this line due to wrong var
				if($tkn ne "((" and $tkn ne "))")
				{
                                        if ($output ne "")
                                        {
